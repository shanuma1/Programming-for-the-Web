...
var Slideous = {
   ...
   registerslides: function() {
      ...
      Slideous.count = 0;
      for (var i in div)
         if (Slideous.hasclass(div[i], "umt-slide"))
   ...
   buildtocs: function() {
      ...
      if (toc) {
         for (var i=0; i<Slideous.count; i++)
            list += Slideous.tocitems.toc.replace(/{\$slideidx}/g, i+1).replace(/{\$slidetitle}/, document.getElementById("s"+(i+1)).getElementsByTagName("h2")[0].innerHTML);
         toc.innerHTML = list;
         while (toc && !Slideous.hasclass(toc, "umt-slide")) toc = toc.parentNode;
         if (toc) Slideous.tocidx = toc.getAttribute("id").substr(1);
      }
      if (tocbox) {
         tocbox.innerHTML = "";
         for (var i=0; i<Slideous.count; i++)
            tocbox.options[tocbox.length] = new Option((i+1)+". "+document.getElementById("s"+(i+1)).getElementsByTagName("h2")[0].innerHTML, "#s"+(i+1));
